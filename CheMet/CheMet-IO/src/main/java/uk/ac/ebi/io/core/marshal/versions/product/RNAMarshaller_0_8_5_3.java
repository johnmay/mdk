 ...
 *
 */
public class RNAMarshaller_0_8_5_3 extends AbstractAnnotatedEntityMarshaller {

    private static final Logger LOGGER = Logger.getLogger(RNAMarshaller_0_8_5_3.class);


    public RNAMarshaller_0_8_5_3() {
        super(new Version(0, 8, 5, 3));
    }


    public RNAProduct read(ReconstructionInputStream in) throws IOException, ClassNotFoundException {

        RNAProduct rna = (RNAProduct) getParent().read(in);

        int n = in.readInt();
        for (int i = 0; i < n; i++) {
            rna.addSequence(SequenceSerializer.readRNASequence(in));
        }
    ...
    public void write(ReconstructionOutputStream out, AnnotatedEntity entity) throws IOException {
        ...
        RNAProduct rna = (RNAProduct) entity;

        // number of sequences
        out.writeInt(rna.getSequences().size());

        // write the sequences
        for (RNASequence sequence : rna.getSequences()) {
