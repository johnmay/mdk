...
public class BrendaParser {
	...
	private int taxid;
	private Vector<BrendaEnzyme> enzymes;
	...
	public BrendaParser(String filename, String organism, String version) throws IOException {
		...
		enzymes = new Vector<BrendaEnzyme>();
		//Enzyme currentEnz = null;
		ArrayList<BrendaEnzyme> currentEnzs = new ArrayList<BrendaEnzyme>();
		...
		while( line != null ) {
			...
			if(line.startsWith("ID")) {
				...
				if(currentEnzs.size() > 0 && organismFound) {
					for(BrendaEnzyme e : currentEnzs) {
						enzymes.add(e);
				...
				currentEnzs = new ArrayList<BrendaEnzyme>();
				currentEnzsIndex = new ArrayList<String>();
				currentEnzs.add(new BrendaEnzyme());
				...
				if(matchSpec.find()) {
					...
					if(proteinAndComments.length() > 7) {
						...
						if(swProtLink.find())
							e.addProteinLink("SwissProt",swProtLink.group(1));
						else if(gbProtLink.find())
							e.addProteinLink("GenBank",gbProtLink.group(1));
						else if(upProtLink.find()) {
							e.addProteinLink("UniProt", upProtLink.group(1));
							auxNumUniProtLinks++;
						}
						else if(tmProtLink.find()) {
							e.addProteinLink("TrEMBL", tmProtLink.group(1));
							auxNumTrEMBLLinks++;
						}
						else if(emProtLink.find()) {
							e.addProteinLink("EMBL", emProtLink.group(1));
							auxNumEMBLLinks++;
						}
				...
				if(tokensSN.length >= 2) {
					//currentEnz.addSystematicName(tokensSN[1]);
					for(BrendaEnzyme e : currentEnzs) {
						e.addSystematicName(tokensSN[1]);
					}
				}
				else {
...
//					currentEnzs.get(indexOfCorrespondingOrg).addSourceTissue(tissue);
				...
				if(line.lastIndexOf("<") > 0 && (line.lastIndexOf("<") < line.lastIndexOf(">")) ) {
					...
					for(int i : orgIndexes ) {
						BrendaEnzyme e = currentEnzs.get(i);
	...
	public Set<String> getAllDistinctMetabolites() {
		...
		for(BrendaEnzyme e : enzymes) {
			for(String m : e.getReaction().getMetabolites() ) {
				metabs.add(m);
			}
		}
		return metabs;
...
//			System.out.println("New Size:\t"+ans.size());
	...
	public Vector<BrendaEnzyme> getEnzymes() {
		return this.enzymes;
	}
