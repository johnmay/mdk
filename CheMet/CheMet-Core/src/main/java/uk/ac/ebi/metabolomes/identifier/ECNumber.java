...
public class ECNumber extends AbstractIdentifier {
    ...
    private String removePostfix( String value , int end ) {
        ...
        return value.substring( 0 , end ).matches( ".*?[-0-9]" ) ? value.substring( 0 , end ) : removePostfix( value , end - 1 );
    }
     ...
     * @return
     */
    private static int compareValues( int ec1ValueArray[] , int ec2ValueArray[] , int index , int matchLevel ) {
        if ( index == ec1ValueArray.length || index == ec2ValueArray.length ) {
            return matchLevel;
        }
        return ec1ValueArray[index] == ec2ValueArray[index] ? compareValues( ec1ValueArray , ec2ValueArray , index + 1 , matchLevel + 1 ) : matchLevel;
    }
