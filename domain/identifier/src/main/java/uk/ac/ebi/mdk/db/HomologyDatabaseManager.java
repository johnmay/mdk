...
public class HomologyDatabaseManager {
    ...
    private static final ResourcePreferences RESOURCE_PREFERENCES = ResourcePreferences.getInstance();
    private static final Pattern NAME_PATTERN = Pattern.compile("(.+)(?:\\.\\d\\d)?\\.(?:phr|pin|pog|psd|psi|psq)");

    private BiMap<String, String> alias = HashBiMap.create();
    ...
    /**
     * Access an alias for the database name. The alias is often more
    ...
    /**
     * Access all files for a specified database name. For example
     * providing "uniprot_sprot" will return "uniprot_sprot.psi",
     * "uniprot_sprot.pin" and the other database files.
     *
    ...
    public Collection<File> getFiles(String key) {
        ...
        if (alias.inverse().containsKey(key)) {
            return files.get(alias.inverse().get(key));
        }
    ...
    /**
     * Check whether the manager has access to a path for a database
     * name or alias.
     *
    ...
    /**
     * Returns the path to the named database. The raw name is first
     * search and then aliases. If not database is found then an
     * {@see InvalidParameterException} is thrown.
     *
     * @param key name or alias (e.g. nr or Non-redundant)
     *
     * @return full path (i.e. getRoot() + name) which can be
     *         used as the database argument when creating a
     *         blast search
     */
    public File getPath(String key) {
        ...
        if (alias.inverse().containsKey(key)) {
            return new File(getRoot(), alias.inverse().get(key));
        }
