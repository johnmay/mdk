 ...
 *
 */
public class HMDBBiofluidsMarshal implements HMDBXMLMarshal {
    ...
    public void marshal(XMLStreamReader2 xmlr, HMDBMetabolite metabolite) throws XMLStreamException {
        // stream until </biofluid_locations>
        while (xmlr.hasNext()) {
            int event = xmlr.next();

            // matches end of element: </biofluid_locations>
            if (event == XMLEvent.END_ELEMENT
                    && tag().equals(xmlr.getLocalName())) {
                return;
            }

            // matches first start of element: <biofluid>...</biofluid>
            if (event == XMLEvent.START_ELEMENT
                    && "biofluid".equals(xmlr.getLocalName())) {
